...
// 释放逻辑地址空间，通过页表得到逻辑空间对应的物理空间并进行释放
void
...
void
freewalk_kernelpgtb(pagetable_t pagetable)
{
  // there are 2^9 = 512 PTEs in a page table.
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];
...
void
ptbprint(pagetable_t pagetable, int level)
{
  // there are 2^9 = 512 PTEs in a page table.
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];
    if (pte & PTE_V) {
      ...
      printf("%d: pte %p pa %p\n", i, pte, child);
      if ((pte & (PTE_R|PTE_W|PTE_X)) == 0) {
        ptbprint((pagetable_t)child, level+1);
      }
    }
...
u2kvmcopy(pagetable_t src, pagetable_t dst, uint64 start, uint64 sz)
{
  ...
  uint64 start_va = PGROUNDUP(start);
  for(i = start_va; i < start + sz; i += PGSIZE){
